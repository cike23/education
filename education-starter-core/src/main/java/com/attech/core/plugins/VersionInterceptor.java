...
public class VersionInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        // 获取StatementHandle，实际是 RoutingStatementHandle
        StatementHandler handle = (StatementHandler) processTarget(invocation.getTarget());
        // 包装原始对象，便于获取和设置属性
        MetaObject metaObject = SystemMetaObject.forObject(handle);
        // MapperStatement 是对 SQL更高层次的封装，这个对象包含了执行SQL所需的各种配置
        ...
        // proceed() 可以执行被拦截对象真正的方法，该方法实际上是执行了method.invoke(target, args)方法
        return invocation.proceed();
    }
